(function(window, document, chrome) {
            ...
            if (isRestricted) {
                ...
                if (!data || Object.keys(data).length === 0) {
                    ...
                    for (let key in data) {
                        ...
                        html += `<tr><td class="td-nome"><input type="text" value="${safeKey}" data-key="${safeKey}" ${currentType==='C'?'readonly':''}></td>
                        <td class="td-value"><input type="text" value="${safeVal}"></td>
                        <td style="text-align:center; white-space:nowrap;"><span class="td-icon open" title="View Detail">${icons.eye}</span><span class="td-icon minus" title="Delete">${icons.trash}</span></td></tr>`;
                    }
                    html += `</tbody></table>`;
                }
            }
    ...
    els.table.addEventListener('click', (e) => {
        ...
        if (icon.classList.contains('minus')) {
            ...
            showConfirm('Delete Item?', `Are you sure you want to delete "${key}"?`, () => { executeAction('remove', {key}, () => { tr.remove(); if(!els.table.querySelector('tbody tr')) renderTable(); showToast('Item deleted'); }); });
